...
class UNet_conditional(nn.Module):
    def __init__(self, initial_in_channels=3, final_out_channels=3, time_dim=256, num_attributes=None, hidden_channels=16, device="cuda"):
        super().__init__()
        self.device = device
        self.time_dim = time_dim
        # Downsample
        self.inc = ConvBlock(initial_in_channels + num_attributes, hidden_channels)
        self.encoder1 = Encoder_block(hidden_channels, hidden_channels*2)
        self.encoder2 = Encoder_block(hidden_channels*2, hidden_channels*4)
        self.encoder3 = Encoder_block(hidden_channels*4, hidden_channels*4)
    ...
    def noise_prediction(self, ins, t, c):
        ...
        if c is not None:
            #print(f"c before: {c.shape}")
            # Unsqueeze and repeat the labels to match the spatial dimensions of the input feature map
            c = c.unsqueeze(2).unsqueeze(3)  # Shape becomes (batch_size, num_attributes, 1, 1)
            c = c.repeat(1, 1, ins.size(2), ins.size(3))  # Repeat along spatial dimensions
            #print(f"c after unsqueeze and repeat: {c.shape}")

            # Concatenate the labels to the input feature maps
            ins = torch.cat([ins, c], dim=1)  # Concatenate along the channel dimension
            #print(f"ins after concatenation: {ins.shape}")
