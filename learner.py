...
class Learner(BaseDifussionLearning):
    ...
    def _hook_every_epoch(self):
        if self.epoch == 0:
            ...
            self.noise = torch.randn(10, 3, 64, 64).to(self.device)
        
        generated_images_fixed, noise_fixed = generate_new_images(self.ddpm, n_samples=10, device=self.device,
                                                channels=3, height=64, width=64, 
                                                denoising_option=self.learner_config["denoising_option"], 
                                                fixed_noise=self.noise)
        
        generated_images, noise_random = generate_new_images(self.ddpm, n_samples=10, device=self.device,
                                                channels=3, height=64, width=64, 
                                                denoising_option=self.learner_config["denoising_option"])
...
class Conditional_Learner(Learner):
    ...
    def _test_epoch(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.test_data):
                ...
                images, labels = data
                images, self.test_labels = images.to(self.device), labels.to(self.device).float()
                ...
                noise_pred = self.noise_prediction(noisy_images, time_steps, self.test_labels)
                ...
                if self.epoch == 0:
                    if i == 0:
                        ...
                        self.noise_tensors_test_initial = noise_tensor
                        self.noises_test_initial = noisy_images
                        self.labels_test_initial = self.test_labels
                
                if i == len(self.test_data) - 2:
                    self.data_storage.dump_store("real_images_test", images)
                    
                    fake_images = generate_images(self.ddpm, self.device, noisy_images, 
                                                  self.learner_config["denoising_option"], self.test_labels)
                    generated_images, _ = generate_new_images(self.ddpm, n_samples=len(images), device=self.device,
                                                            channels=3, height=64, width=64, 
                                                            denoising_option=self.learner_config["denoising_option"],
                                                            labels=self.test_labels,
                    ...
                    self.data_storage.dump_store("noise_tensors_test", noise_tensor)
                    self.data_storage.dump_store("noise_images_test", noisy_images)
                    self.data_storage.dump_store("labels_test", self.test_labels)
    ...
    def _hook_every_epoch(self):
        if self.epoch == 0:
            ...
            self.noise = torch.randn(30, 3, 64, 64).to(self.device)
            # Replace label selection with random label generation for CelebA
            self.testing_labels = torch.randint(0, 2, (30, 10)).to(self.device)
        
        generated_images_fixed, noise_fixed = generate_new_images(self.ddpm, n_samples=30, device=self.device,
                                                channels=3, height=64, width=64, 
                                                denoising_option=self.learner_config["denoising_option"], 
                                                fixed_noise=self.noise,
                                                labels=self.testing_labels)
        
        generated_images, noise_random = generate_new_images(self.ddpm, n_samples=30, device=self.device,
                                                channels=3, height=64, width=64, 
                                                denoising_option=self.learner_config["denoising_option"],
                                                labels=self.testing_labels)
